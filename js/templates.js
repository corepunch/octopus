...
<div class="post-card post-card--{{postType}}">
  ...
  {{else if (eq postType "quote")}}
    ...
    <div class="post-card-body">
      ...
      {{#if userText}}<p class="post-card-user-text">{{userText}}</p>{{/if}}
  ...
  {{else if (eq postType "link")}}
    <div class="post-card-body">
      {{#if linkUrl}}
        {{#if title}}
          <a href="{{safeUrl linkUrl}}" class="post-card-title post-card-title--link" target="_blank" rel="noopener noreferrer">{{icon "link"}} {{title}}</a>
          <div class="post-card-link-domain">{{displayUrl linkUrl}}</div>
        {{else}}
          <a href="{{safeUrl linkUrl}}" class="post-card-title post-card-title--link" target="_blank" rel="noopener noreferrer">{{icon "link"}} {{displayUrl linkUrl}}</a>
        {{/if}}
      {{else}}
        <a href="post.html?id={{id}}" class="post-card-title">Link</a>
      {{/if}}
      <div class="post-meta">
        by <a href="profile.html?id={{urlEncode authorId}}" class="author-link">{{authorName}}</a>
        · {{timeAgo createdAt}}
...
{{else if (eq postType "link")}}
  <h1 class="post-page-title">
    {{#if linkUrl}}
      {{#if title}}
        <a href="{{safeUrl linkUrl}}" class="post-page-title-link" target="_blank" rel="noopener noreferrer">{{icon "link"}} {{title}}</a>
      {{else}}
        <a href="{{safeUrl linkUrl}}" class="post-page-title-link" target="_blank" rel="noopener noreferrer">{{icon "link"}} {{displayUrl linkUrl}}</a>
      {{/if}}
    {{else}}
      Link
    {{/if}}
  </h1>
  {{#if title}}{{#if linkUrl}}<div class="post-page-link-domain">{{displayUrl linkUrl}}</div>{{/if}}{{/if}}
  <div class="post-page-meta">
    By <a href="profile.html?id={{urlEncode authorId}}" class="author-link">{{authorName}}</a>
    · {{timeAgo createdAt}}
    {{#each tags}}<a href="search.html?tag={{urlEncode this}}" class="tag">#{{this}}</a>{{/each}}
...
<div class="widget">
  ...
  <div style="margin-bottom:8px;">
    <a href="post.html?id={{urlEncode id}}">{{label}}</a>
  </div>
  {{/each}}
</div>`;
